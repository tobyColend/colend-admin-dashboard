...
async function processToken(address, label, symbol, originBlock = 0, latestBlock, priceMap, withUSD) {
    ...
    for (const [addr, entry] of Object.entries(cache.holders)) {
        const raw = typeof entry === "string" ? BigInt(entry) : BigInt(entry.raw);
        const usd = typeof entry === "string" ? 0 : entry.usd || 0;
    ...
    return {
        [`tvl${label === "a" ? "" : label.charAt(0).toUpperCase() + label.slice(1)}Raw`]: totalRaw.toString(),
        [`tvl${label === "a" ? "" : label.charAt(0).toUpperCase() + label.slice(1)}USD`]: totalUSD,
        [`${label}HolderCount`]: count,
        [`${label}Holders`]: cache.holders
    };
}
...
async function fetchHolderCounts(withUSD = false) {
    ...
    for (const asset of assets) {
        ...
        try {
            ...
            if (asset.debtToken) {
                results[symbol].debtToken = await processToken(
                    asset.debtToken,
                    "aDebt",
                    symbol,
                    asset.originBlock,
